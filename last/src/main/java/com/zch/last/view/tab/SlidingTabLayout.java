...
public class SlidingTabLayout extends HorizontalScrollView {
    ...
    private void set(Context context, AttributeSet attrs, int defStyle) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SlidingTabLayout);
        mTabViewLayoutId = typedArray.getResourceId(R.styleable.SlidingTabLayout_tab_layout, -1);
        mTabViewTextViewId = typedArray.getResourceId(R.styleable.SlidingTabLayout_tab_id, -1);
        typedArray.recycle();
    }
    ...
    public void notifyTabRemove(int position) {
        if (mTabStrip.mSelectedPosition >= position) {
            mTabStrip.mSelectedPosition--;
        }
        populateTabStrip();
    }
    ...
    public void populateTabStrip() {
        ...
        if (mTabStrip.mSelectedPosition >= adapter.getCount()) {
            mTabStrip.mSelectedPosition = adapter.getCount() - 1;
        }
    ...
    private View.OnLongClickListener tabLongClickListener = new OnLongClickListener() {
        @Override
        public boolean onLongClick(View v) {
            for (int i = 0; i < mTabStrip.getChildCount(); i++) {
                if (v == mTabStrip.getChildAt(i)) {
                    if (onTabListener != null) {
                        onTabListener.onLongClick(v, i);
                    }
                    return true;
                }
            }
